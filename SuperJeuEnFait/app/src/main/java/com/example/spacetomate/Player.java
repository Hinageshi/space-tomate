...
public class Player {
    ...
    public Player(String n){
        ...
        stockProducts=new int[Game.numberOfProducts];
        stockDust=new int[Game.numberOfProducts];
        for(int i=0;i<Game.numberOfProducts;i++){
            stockDust[i]=0;
            stockProducts[i]=0;
        }
    ...
    public void addDusttStock(Products p,int quantity){
        switch (p){
            case TOMATO:
    ...
    public void unlockNewProduct(){
       if(unlockedProducts<Game.numberOfProducts){
           spaceship.unlockNextGarden();
           unlockedProducts++;
       }
    }

    //price will be calculated in the Game class
